...
var agenda = new Agenda({ db: { address: process.env.MONGODB_URL, collection: 'agenda_jobs' } });
...
agenda.define('queryGoogle', function(job, done) {
  ...
  queryInterface.listUserAccounts(function(err, response) {
    ...
    var account = getByName(response.items, name);

    if (!account) {
      return done(new Error('No account name matched ' + name));
    }
    ...
    queryInterface.listWebProperties(accountId, function(err, response) {
      ...
      var property = getByName(response.items, name);

      if (!property) {
        return done(new Error('No property name matched ' + name));
      }
      ...
      queryInterface.listProfiles(accountId, propertyId, function(err, response) {
        ...
        var profile = getByName(response.items, name);

        if (!profile) {
          return done(new Error('No profile name matched ' + name));
        }
